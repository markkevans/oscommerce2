    ...
    switch ($$db_type)
    {
        ...
        case 'sqlsrv':
            ...
            (
                ...
                'UID' => $username,
                'PWD' => $password,
                "ConnectionPooling" => true, // Optional.
            );

            $$link = sqlsrv_connect($server, $conn_array);

            if (!is_resource($$link)) {
                    osc_db_error_sqlsrv('osc_db_query', $query); // die
            }
...
// msk
  ...
  function osc_db_query_sqlsrv($query, $link = 'db_link') {
    ...
    if (!$link) {
        osc_db_error_sqlsrv('Database not connected'); // die
    }
    ...
    /*
    $myFile = "c:\inetpub\wwwroot\zencart\includes\work\queryLog.txt";
    $fh = fopen($myFile, 'a+') or die("can't open file");
    $crlf = chr(13).chr(10);
    fwrite($fh, $crlf.'Query=> '.$zf_sql);
    fclose($fh);
    */
...
//    $count_queries++;
  ...
  function _LimitToTopN($sql)
  {
        ...
        foreach ($parts as $part) {
            ...
            if (($topN != null) && ($partLo == 'select') || ($partLo == 'delete') || ($partLo == 'update') || ($partLo == 'insert')) {
                if ($disctinct_found) {
                    $part .= ' DISTINCT ';
                    $disctinct_found = false;
                } else if ($all_found) {
