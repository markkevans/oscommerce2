...
function tep_db_query($query, $link = 'db_link') {
    ...
    if (!is_resource($$link)) {
        tep_db_error('Database not connected');
    }
    ...
    $db_resource =
        sqlsrv_query($$link, trim($query), array (), array ("Scrollable" => SQLSRV_CURSOR_KEYSET));
...
function tep_db_backup($backup_file, $database = DB_DATABASE, $desc, $link = 'db_link') {
    ...
    $query =
        "BACKUP DATABASE ".DB_DATABASE." TO DISK = '".$backup_file."' WITH DESCRIPTION = '".$desc
        ."'";
...
function tep_db_insert_id($link = 'db_link') {
    ...
    $db_resource = sqlsrv_query($$link, $query);
    if ($db_resource === false || !is_resource($db_resource)) {
        if (defined('STORE_DB_TRANSACTIONS') && (STORE_DB_TRANSACTIONS == 'true')) {
            $logger->write(tep_db_format_error(), 'ERROR');
        }
        tep_db_error('tep_db_insert_id', $query);
    }

    $row = sqlsrv_fetch_array($db_resource, SQLSRV_FETCH_NUMERIC);
    sqlsrv_free_stmt($db_resource);
...
function tep_db_error($err_text = '', $query, $fatal = true) {
    ...
    if ($fatal) {
        die('<font color="#000000"><b>'.$errno.' - '.$error.'<br><br>'.$query
